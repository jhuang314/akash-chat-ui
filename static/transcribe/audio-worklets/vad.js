...
class AudioVADProcessor extends AudioWorkletProcessor {
	...
	constructor(options) {
		...
		this.last_command_was_speech =
			options.processorOptions.lastCommandWasSpeech ?? this.last_command_was_speech;
	...
	process(inputs, outputs, parameters) {
		...
		sfm =
			-10 *
			Math.log10(
				Math.exp(sfm_sum_geo / frequencyData.length) / (sfm_sum_ari / frequencyData.length)
			);
		...
		if (count > 1) {
			...
			this.e_min =
				(this.is_silent_frame_counter * this.e_min + energy) / (this.is_silent_frame_counter + 1);
