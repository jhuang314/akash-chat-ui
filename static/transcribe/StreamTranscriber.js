...
export class StreamTranscriber extends Transcriber {
	...
	constructor(options) {
		...
		this._audioWorkletsPath = options.audioWorkletsPath?.trim().replace(/\/$/, "") ?? "";
	...
	async init() {
		...
		await this._streamAudioContext.audioWorklet.addModule(this.getAudioWorkletPath("vad.js"));
		await this._streamAudioContext.audioWorklet.addModule(this.getAudioWorkletPath("buffer.js"));
	...
	async transcribe(
		stream,
		{ preRecordMs = 200, maxRecordMs = 5000, minSilenceMs = 500, onVoiceActivity = null } = {}
	) {
		if (!this.isStreamRunning) {
			console.log("Stream not running.");
			return;
		...
		try {
			...
			const preRecordSamplesCnt = (preRecordMs * this._streamAudioContext.sampleRate) / 1000;

			const maxRecordSamplesCnt = (maxRecordMs * this._streamAudioContext.sampleRate) / 1000;

			this._streamMediaSource = this._streamAudioContext.createMediaStreamSource(stream);
			...
			const vad = createVad(this._streamAudioContext, onSpeech, onSilence, minSilenceMs);
